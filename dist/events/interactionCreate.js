...
const event = {
    ...
    async execute(client, interaction) {
        if (!interaction.channel) {
            throw new Error('Interaction channel cannot be found.');
        }
        ...
        if (interaction.isModalSubmit()) {
            if (interaction.customId.includes('displayedspecies')) {
                ...
                userData = /** @type {import('../typedef').ProfileSchema} */ (await profileModel.findOneAndUpdate({ userId: userId }, (/** @type {import('../typedef').ProfileSchema} */ p) => {
        ...
        if ((interaction.isButton() || interaction.isSelectMenu()) && !interaction.customId.includes('modal')) {
            ...
            if (interaction.isButton() && interaction.customId === 'ticket' && interaction.channel.type === 'DM') {
                ...
                if (!user.dmChannel) {
                    throw new Error('User\'s DM Channel could not be found.');
                }
            ...
            if (interaction.isSelectMenu()) {
                ...
                if (interaction.customId.includes('help')) {
                    sendInteractionResponse(client, interaction);
                }
            }
