...
const event = {
    ...
    async execute(client, role) {
        ...
        for (const shoprole of roles) {
            const allServerUsers = /** @type {Array<import('../typedef').ProfileSchema>} */ (await profileModel.find((/** @type {import('../typedef').ProfileSchema} */ p) => Object.keys(p.currentCharacter).includes(role.guild.id)));
            for (const user of allServerUsers) {
                ...
                for (const character of characters) {
                    ...
                    if (profile.roles.some(profilerole => profilerole.roleId === shoprole.roleId && profilerole.wayOfEarning === shoprole.wayOfEarning && profilerole.requirement === shoprole.requirement)) {
                        ...
                        if (userRoleIndex >= 0) {
                            profile.roles.splice(userRoleIndex, 1);
                        }
                        await profileModel.findOneAndUpdate({ uuid: user.uuid }, (/** @type {import('../typedef').ProfileSchema} */ p) => {
