...
async function addFriendshipPoints(message, userData, characterId, partnerUserData, partnerCharacterId) {
    ...
    userData = /** @type {import('../typedef').ProfileSchema} */ (await profileModel.findOneAndUpdate({ uuid: userData.uuid }, (/** @type {import('../typedef').ProfileSchema} */ p) => {
        if (p.characters[characterId].mentions[partnerCharacterId] === undefined) {
            ...
            p.characters[characterId].mentions[partnerCharacterId].push(message.createdTimestamp);
        }
    }));
...
async function checkOldMentions(userData, characterId, partnerUserData, partnerCharacterId) {
    ...
    partnerUserData = /** @type {import('../typedef').ProfileSchema} */ (await profileModel.findOneAndUpdate({ uuid: partnerUserData.uuid }, (/** @type {import('../typedef').ProfileSchema} */ p) => {
