...
module.exports.sendMessage = async (client, message, argumentsArray, userData, serverData, embedArray) => {
    ...
    if (!hasCompletedAccount(message, characterData) || await isInvalid(message, userData, embedArray, [module.exports.name])) {
        return;
    }
    ...
    for (let i = 0; i < 5; i++) {
        ...
        for (let j = 0; j < 5; j++) {
            ...
            if (i === correctCoordinates[0] && j === correctCoordinates[1]) {
                gamePositionsArray[i].push(filledFieldArray[0]);
            }
            else if (Math.abs(i - correctCoordinates[0]) <= 1 && Math.abs(j - correctCoordinates[1]) <= 1) {
                gamePositionsArray[i].push(filledFieldArray[1]);
            }
            else if (Math.abs(i - correctCoordinates[0]) <= 2 && Math.abs(j - correctCoordinates[1]) <= 2) {
                gamePositionsArray[i].push(filledFieldArray[2]);
            }
            else if (Math.abs(i - correctCoordinates[0]) <= 3 && Math.abs(j - correctCoordinates[1]) <= 3) {
                gamePositionsArray[i].push(filledFieldArray[3]);
            }
            else {
                gamePositionsArray[i].push(filledFieldArray[4]);
            }
        }
    }
        ...
        await /** @type {Promise<void>} */ (new Promise((resolve) => {
            ...
            collector.on('collect', async (interaction) => {
                ...
                if (interaction.customId.includes('board-')) {
                    ...
                    if (interaction.customId.includes(humanTrapCorrectEmoji)) {
