...
module.exports.sendMessage = async (client, message, argumentsArray, userData, serverData, embedArray) => {
    ...
    async function startQuest() {
        ...
        userData = /** @type {import('../../typedef').ProfileSchema} */ (await profileModel.findOneAndUpdate({ userId: message.author.id }, (/** @type {import('../../typedef').ProfileSchema} */ p) => {
            ...
            p.characters[characterData._id].profiles[message.guildId].hasQuest = false;
        }));
        let hitEmoji = '';
        let missEmoji = '';
        let hitValue = 1;
        ...
        async function startNewRound() {
            ...
            if (customId !== '') {
                ...
                if (buttonIndex >= 0) { /** @type {import('discord.js').MessageButton} */
                    (botReply.components[botReply.components.length - 1].components[buttonIndex]).setEmoji('🔘');
                }
            }
            if (randomNumber <= winChance) {
                ...
                if (buttonIndex >= 0) { /** @type {import('discord.js').MessageButton} */
                    (botReply.components[botReply.components.length - 1].components[buttonIndex]).setEmoji('☑️');
                }
            }
            ...
            if (hitValue >= 10) {
                if (profileData.unlockedRanks < 3) {
                    await profileModel.findOneAndUpdate({ userId: message.author.id }, (/** @type {import('../../typedef').ProfileSchema} */ p) => {
                        // @ts-ignore, as message is must be in server
                        p.characters[characterData._id].profiles[message.guildId].unlockedRanks += 1;
                    });
                }
                ...
                if (profileData.rank === 'Elderly') {
                    ...
                    await profileModel.findOneAndUpdate({ userId: message.author.id }, (/** @type {import('../../typedef').ProfileSchema} */ p) => {
                        // @ts-ignore, as message is must be in server
                        p.characters[characterData._id].profiles[message.guildId].maxHealth += maxHealthPoints;
                        // @ts-ignore, as message is must be in server
                        p.characters[characterData._id].profiles[message.guildId].maxEnergy += maxEnergyPoints;
                        // @ts-ignore, as message is must be in server
                        p.characters[characterData._id].profiles[message.guildId].maxHunger += maxHungerPoints;
                        // @ts-ignore, as message is must be in server
                        p.characters[characterData._id].profiles[message.guildId].maxThirst += maxThirstPoints;
                    });
                }
