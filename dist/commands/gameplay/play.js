...
module.exports.sendMessage = async (client, message, argumentsArray, userData, serverData, embedArray) => {
    ...
    userData = /** @type {import('../../typedef').ProfileSchema} */ (await profileModel.findOneAndUpdate({ userId: message.author.id }, (/** @type {import('../../typedef').ProfileSchema} */ p) => {
    ...
    if (!firstMentionedUser) {
        const allPrairieUsersList = /** @type {Array<import('../../typedef').ProfileSchema>} */ (await profileModel
            .find((/** @type {import('../../typedef').ProfileSchema} */ p) => {
        ...
        if (generateRandomNumber(3, 0) === 0 && profileData.unlockedRanks === 0 && profileData.rank === 'Youngling' && profileData.levels > 1 && profileData.hasQuest === false) {
            ...
            partnerCharacterData = Object.values(partnerUserData?.characters || {}).find(c => c?.profiles?.[message.guild.id] !== undefined && c?.profiles?.[message.guild.id].currentRegion === 'prairie' && c?.profiles?.[message.guild.id]?.energy > 0 && c?.profiles?.[message.guild.id]?.health > 0 && c?.profiles?.[message.guild.id]?.hunger > 0 && c?.profiles?.[message.guild.id]?.thirst > 0 && c?.profiles?.[message.guild.id]?.injuries?.cold === false) || null;
            partnerProfileData = partnerCharacterData?.profiles?.[message.guild.id] || null;
    ...
    if (partnerUserData && partnerCharacterData) {
        await addFriendshipPoints(message, userData, characterData._id, partnerUserData, partnerCharacterData._id);
    }
    ...
    async function findQuest() {
        await profileModel.findOneAndUpdate({ userId: message.author.id }, (/** @type {import('../../typedef').ProfileSchema} */ p) => {
            ...
            p.characters[p.currentCharacter[message.guildId]].profiles[message.guildId].hasQuest = true;
        });
    ...
    async function findPlantOrNothing() {
        ...
        if (!userSpeciesMap) {
            throw new Error('userSpeciesMap is missing');
        }
        ...
        if (correctButtonIndex >= 0) { /** @type {import('discord.js').MessageButton} */
            (botReply.components[botReply.components.length - 1].components[correctButtonIndex]).style = 'PRIMARY';
        }
        if (customId !== '') {
            ...
            if (buttonIndex >= 0) { /** @type {import('discord.js').MessageButton} */
                (botReply.components[botReply.components.length - 1].components[buttonIndex]).style = 'DANGER';
            }
        }
        embed.footer.text = embedFooterStatsText;
        if (customId?.includes(`${correctButton}`) === true) {
            ...
            const buttonIndex = botReply.components[botReply.components.length - 1].components.findIndex(button => button.customId === customId);
            if (buttonIndex >= 0) { /** @type {import('discord.js').MessageButton} */
                (botReply.components[botReply.components.length - 1].components[buttonIndex]).style = 'SUCCESS';
            }
            const userInventory = {
    ...
    async function playTogether(isMentioned, isSimulated) {
        if (!isSimulated && (profileData.rank === 'Youngling' || profileData.rank === 'Apprentice')) {
            // @ts-ignore, since partnerData can't be null
            ...
            // @ts-ignore, since partnerData can't be null
            { userId: partnerUserData.userId }, (/** @type {import('../../typedef').ProfileSchema} */ p) => {
                // @ts-ignore, since partnerData can't be null and message must be in guild
                p.characters[partnerCharacterData._id].profiles[message.guildId].health += partnerHealthPoints;
            }));
            // @ts-ignore, since partnerData can't be null
        ...
        if (!isSimulated && partnerProfileData.injuries.cold === true && profileData.injuries.cold === false && pullFromWeightedTable({ 0: 3, 1: 7 }) === 0) {
            ...
            botReply = await botReply
                ...
                return /** @type {import('discord.js').Message} */ (botReply);
            });
        }
        return botReply;
    }
