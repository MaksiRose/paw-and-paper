...
module.exports.sendMessage = async (client, message, argumentsArray, userData, serverData, embedArray) => {
    ...
    ][['cold', 'warm', 'water'].indexOf(userSpeciesMap.habitat)].slice(0, (profileData.rank == 'Elderly') ? 3 : (profileData.rank == 'Healer' || profileData.rank == 'Hunter') ? 2 : 1);
    ...
    collector.on('collect', async (interaction) => {
        if (newPushpinPosition.vertical !== null && newPushpinPosition.horizontal !== null && waitingArray[newPushpinPosition.vertical][newPushpinPosition.horizontal] === '📍') {
            if (interaction.customId === 'explore-left') {
                newPushpinPosition.horizontal -= 1;
            }
            if (interaction.customId === 'explore-up') {
                newPushpinPosition.vertical -= 1;
            }
            if (interaction.customId === 'explore-down') {
                newPushpinPosition.vertical += 1;
            }
            if (interaction.customId === 'explore-right') {
                newPushpinPosition.horizontal += 1;
            }
            let options = [
    ...
    const highRankProfilesCount = /** @type {Array<import('../../typedef').ProfileSchema>} */ (await profileModel
        .find((/** @type {import('../../typedef').ProfileSchema} */ u) => Object.values(u.characters).filter(c => c.profiles[message.guild.id] !== undefined && c.profiles[message.guild.id].rank !== 'Youngling').length > 0))
    ...
    async function findPlant() {
        ...
        async function pickupCycle(totalCycles, lastRoundEmojiIndex, winPoints) {
            ...
            const allElderlyUsersArray = /** @type {Array<import('../../typedef').ProfileSchema>} */ (await profileModel.find((/** @type {import('../../typedef').ProfileSchema} */ u) => {
                // @ts-ignore, since message is safe to be in guild
    ...
    async function findEnemy() {
        ...
        if (chosenBiomeNumber > 0) {
            opponentsArray.push(...userSpeciesMap?.biome2OpponentArray || []);
        }
        if (chosenBiomeNumber === 2) {
            opponentsArray.push(...userSpeciesMap?.biome3OpponentArray || []);
        }
