...
module.exports.sendMessage = async (client, message, argumentsArray, userData, serverData) => {
        ...
        await botReply
            ...
            if (interaction.isSelectMenu() && message.inGuild()) {
                if (interaction.values[0].includes('page')) {
                    ...
                    if (channelId === 'everywhere') {
                        argumentsArray[0] = channelId;
                    }
                    // @ts-ignore, since that channelId variable is derived from the allChannels Collection
                    else {
                        message.mentions.channels.set(channelId, allChannels.get(channelId));
                    }
                    await alwaysProxy();
                }
    ...
    async function alwaysProxy() {
        ...
        if ((firstMentionedChannel && firstMentionedChannel.isText()) || autoproxy === 'everywhere') {
            ...
            userData = /** @type {import('../../typedef').ProfileSchema} */ (await profileModel.findOneAndUpdate({ uuid: userData.uuid }, (/** @type {import('../../typedef').ProfileSchema} */ p) => {
                if (p.autoproxy[message.guild.id] === undefined) {
                    p.autoproxy[message.guild.id] = [];
                }
                if (hasChannel) {
                    p.autoproxy[message.guild.id] = p.autoproxy[message.guild.id].filter(string => string !== autoproxy);
                }
                else {
                    p.autoproxy[message.guild.id].push(autoproxy);
                }
            }));
            await message
                .reply({
    ...
    async function disableProxy() {
        ...
        if (((firstMentionedChannel && firstMentionedChannel.isText()) || place === 'everywhere') && (kind === 'all' || kind === 'auto')) {
            ...
            serverData = /** @type {import('../../typedef').ServerSchema} */ (await serverModel.findOneAndUpdate({ serverId: message.guild.id }, (/** @type {import('../../typedef').ServerSchema} */ s) => {
                if (hasChannel) {
                    s.proxysetting[kind] = s.proxysetting[kind].filter(string => string !== place);
                }
                else {
                    s.proxysetting[kind].push(place);
                }
            }));
