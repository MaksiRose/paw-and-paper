...
module.exports.sendMessage = async (client, message, argumentsArray, userData, serverData) => {
    ...
    await serverModel.findOneAndUpdate({ serverId: serverData.serverId }, (/** @type {import('../../typedef').ServerSchema} */ s) => {
        s.currentlyVisiting = null;
    });
    await serverModel.findOneAndUpdate({ serverId: otherServerData.serverId }, (/** @type {import('../../typedef').ServerSchema} */ s) => {
        s.currentlyVisiting = null;
    });
    await thisChannel
        .send({
