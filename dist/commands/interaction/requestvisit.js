...
module.exports.sendMessage = async (client, message, argumentsArray, userDataV, serverDataV) => {
    ...
    let visitableServers = /** @type {Array<import('../../typedef').ServerSchema>} */ (await serverModel.find((/** @type {import('../../typedef').ServerSchema} */ s) => {
    ...
    async function interactionCollector() {
        ...
        if (interaction.values[0] == 'visit_page') {
            visitableServers = /** @type {Array<import('../../typedef').ServerSchema>} */ (await serverModel.find((/** @type {import('../../typedef').ServerSchema} */ s) => {
        ...
        if (interaction.values[0].startsWith('visit-')) {
            ...
            let serverDataH = /** @type {import('../../typedef').ServerSchema} */ (await serverModel.findOne({ serverId: hostGuildId }));
            ...
            serverDataV = /** @type {import('../../typedef').ServerSchema} */ (await serverModel.findOneAndUpdate({ serverId: serverDataV.serverId }, (/** @type {import('../../typedef').ServerSchema} */ s) => { s.currentlyVisiting = serverDataH.serverId; }));
            serverDataH = /** @type {import('../../typedef').ServerSchema} */ (await serverModel.findOneAndUpdate({ serverId: serverDataH.serverId }, (/** @type {import('../../typedef').ServerSchema} */ s) => { s.currentlyVisiting = serverDataV.serverId; }));
            await botReplyV
                .edit({
                components: disableAllComponents(botReplyV.components),
            })
...
async function declinedInvitation(message, characterData, botReplyV, botReplyH) {
    ...
    await serverModel.findOneAndUpdate({ serverId: botReplyV.guildId }, (/** @type {import('../../typedef').ServerSchema} */ s) => { s.currentlyVisiting = null; });
    await serverModel.findOneAndUpdate({ serverId: botReplyH.guildId }, (/** @type {import('../../typedef').ServerSchema} */ s) => { s.currentlyVisiting = null; });
}
/**
 *
...
async function acceptedInvitation(client, message, botReplyV, botReplyH, serverDataV, serverDataH, characterDataV, characterDataH) {
    ...
    async function collectMessages(thisServerChannel, otherServerChannel) {
        ...
        collector.on('end', async () => {
            const thisServerData = /** @type {import('../../typedef').ServerSchema} */ (await serverModel.findOne({ serverId: thisServerChannel.guild.id }));
            const otherServerData = /** @type {import('../../typedef').ServerSchema} */ (await serverModel.findOne({ serverId: otherServerChannel.guild.id }));
            if (thisServerData.currentlyVisiting !== null && otherServerData.currentlyVisiting !== null) {
                await serverModel.findOneAndUpdate({ serverId: thisServerChannel.guild.id }, (/** @type {import('../../typedef').ServerSchema} */ s) => { s.currentlyVisiting = null; });
                await serverModel.findOneAndUpdate({ serverId: otherServerChannel.guild.id }, (/** @type {import('../../typedef').ServerSchema} */ s) => { s.currentlyVisiting = null; });
...
module.exports.sendVisitMessage = async (client, message, userData, thisServerData, otherServerData) => {
    ...
    if (!webhookChannel) {
        throw new Error('Webhook can\'t be edited, interaction channel is thread and parent channel cannot be found');
    }
