        ...
        if (!isFirstPick) {
            ...
            setTimeout(async () => {
                ...
                if (componentArray.every(actionRow => actionRow.components.every(button => button.disabled === true))) {
                    ...
                    winningUserData = /** @type {import('../../typedef').ProfileSchema} */ (await profileModel.findOneAndUpdate({ userId: winningUserData.userId }, (/** @type {import('../../typedef').ProfileSchema} */ p) => {
                        // @ts-ignore, since message must be in guild
                    ...
                    losingUserData = /** @type {import('../../typedef').ProfileSchema} */ (await profileModel.findOneAndUpdate({ userId: losingUserData.userId }, (/** @type {import('../../typedef').ProfileSchema} */ p) => {
                        // @ts-ignore, since message must be in guild
...
async function decreaseStats(message, userData, profileData, partnerUserData, partnerProfileData) {
    ...
    { userId: message.mentions.users.first().id }, (/** @type {import('../../typedef').ProfileSchema} */ p) => {
        // @ts-ignore, since message must be in guild
