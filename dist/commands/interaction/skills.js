...
module.exports.sendMessage = async (client, message, argumentsArray, userData, serverData) => {
    ...
    userData = /** @type {import('../../typedef').ProfileSchema} */ (await profileModel.findOneAndUpdate({ userId: userData.userId }, (/** @type {import('../../typedef').ProfileSchema} */ p) => {
        if (characterData && profileData) {
            p.characters[characterData._id].profiles[profileData.serverId].skills.global = newGlobalSkills;
        }
    }));
    characterData = userData?.characters?.[characterData?._id];
    profileData = characterData?.profiles?.[message.guildId];
    /* Creating a message with 4 buttons and a skill list. */
    const basicButtons = new MessageActionRow().addComponents([new MessageButton({
            customId: 'skills-add',
            label: 'Add',
            emoji: '✏️',
            style: 'SECONDARY',
    ...
    async function interactionCollector() {
        ...
        await botReply
            ...
            if (interaction.isButton() && interaction.customId === 'skills-modify') {
                ...
                if (modifyMenu.options.length > 25) {
                    modifyMenu.options.length = 25;
                }
                await /** @type {import('discord.js').Message} */ (interaction.message)
                    .edit({
                    components: [
            ...
            if (interaction.isSelectMenu() && interaction.customId === 'skills-remove-options') {
                ...
                if (category === 'personal') {
                    ...
                    for (const user of allUsers) {
                        await profileModel.findOneAndUpdate({ userId: user.userId }, (/** @type {import('../../typedef').ProfileSchema} */ p) => {
                    ...
                    await serverModel.findOneAndUpdate({ serverId: message.guildId }, (/** @type {import('../../typedef').ServerSchema} */ s) => {
            ...
            interaction.awaitModalSubmit({ filter: i => i.customId.includes('skill'), time: 120_000 })
                ...
                await i
                    ...
                    if (error.httpStatus === 400) {
                        return console.error('DiscordAPIError: Interaction has already been acknowledged.');
                    }
                    if (error.httpStatus === 404) {
                        return console.error('DiscordAPIError: Unknown interaction. (This probably means that there was server-side delay when receiving the interaction)');
                    }
                });
                ...
                if (type === 'add') {
                    ...
                    if (category === 'personal') {
                        ...
                        userData = /** @type {import('../../typedef').ProfileSchema} */ (await profileModel.findOneAndUpdate({ userId: i.user.id }, (/** @type {import('../../typedef').ProfileSchema} */ p) => {
                            // @ts-ignore, since message must be in guild
                        ...
                        await serverModel.findOneAndUpdate({ serverId: i.guildId }, (/** @type {import('../../typedef').ServerSchema} */ s) => {
                    ...
                    userData = /** @type {import('../../typedef').ProfileSchema} */ (await profileModel.findOneAndUpdate({ userId: i.user.id }, (/** @type {import('../../typedef').ProfileSchema} */ p) => {
                        // @ts-ignore, since message must be in guild
                        if (plusOrMinus === '+') {
                            p.characters[p.currentCharacter[i.guildId]].profiles[i.guildId].skills[category][name] += newValue;
                        }
                        // @ts-ignore, since message must be in guild
                        else if (plusOrMinus === '-') {
                            p.characters[p.currentCharacter[i.guildId]].profiles[i.guildId].skills[category][name] -= newValue;
...
function getSkillList(profileData) {
    ...
    if (skillList === '') {
        skillList = 'There is nothing to show here :(';
    }
    return skillList;
}
