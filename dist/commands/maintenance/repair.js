...
module.exports.sendMessage = async (client, message, argumentsArray, userData, serverData, embedArray) => {
    ...
    let chosenDen = ['sleeping dens', 'food den', 'medicine den'].includes(argumentsArray.join(' ').toLowerCase()) ?
        ['sleepingDens', 'foodDen', 'medicineDen'][['sleeping dens', 'food den', 'medicine den'].indexOf(argumentsArray.join(' ').toLowerCase())] : null;
    const denSelectMenu = new MessageActionRow().addComponents([new MessageButton({
    ...
    async function interactionCollector() {
        ...
        if (interaction.isSelectMenu()) {
            ...
            serverData = /** @type {import('../../typedef').ServerSchema} */ (await serverModel.findOneAndUpdate({ serverId: interaction.guild.id }, (/** @type {import('../../typedef').ServerSchema} */ s) => {
                s.inventory.materials[interaction.values[0]] -= 1;
                // @ts-ignore, since select menus always have a chosen den
                if (isSuccessful) {
                    s.dens[chosenDen][repairKind] += repairAmount;
                }
            }));
    ...
    async function getMaterials() {
        ...
        const embeds = [...embedArray, embed, embed2], components = [denSelectMenu, ...selectMenu.options.length > 0 ? [new MessageActionRow().addComponents([selectMenu])] : []];
