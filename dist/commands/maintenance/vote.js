...
module.exports.sendMessage = async (client, message, argumentsArray, userData, serverData, embedArray) => {
    ...
    botReply
        ...
        if (successfulTopVote === true || successfulDiscordsVote === true || successfulDblVote === true) {
            ...
            if (successfulTopVote === true) {
                voteCache['id_' + message.author.id].nextRedeemableTopVote = (voteCache['id_' + message.author.id].lastRecordedTopVote || Date.now()) + twelveHoursInMs;
            }
            if (successfulDiscordsVote === true) {
                voteCache['id_' + message.author.id].nextRedeemableDiscordsVote = voteCache['id_' + message.author.id]?.lastRecordedDiscordsVote > Date.now() - twelveHoursInMs ? voteCache['id_' + message.author.id].lastRecordedDiscordsVote + twelveHoursInMs : Number(discordsVote.votes[0].expires);
            }
            if (successfulDblVote === true) {
                voteCache['id_' + message.author.id].nextRedeemableDblVote = voteCache['id_' + message.author.id].lastRecordedDblVote + twelveHoursInMs;
            }
