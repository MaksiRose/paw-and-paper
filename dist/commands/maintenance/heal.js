...
module.exports.sendMessage = async (client, message, argumentsArray, userData, serverData, embedArray) => {
    ...
    allHurtCharactersList = {}, currentUserPage = 0, userSelectMenu = await getUserSelectMenu(message), chosenUserData = message.mentions.users.size > 0 ?
        ...
        Object.keys(allHurtCharactersList).length === 1 ?
            /** @type {import('../../typedef').ProfileSchema} */ (await profileModel.findOne({ userId: Object.keys(allHurtCharactersList)[0].split('_')[0] })) : null, chosenCharacterData = chosenUserData ? chosenUserData?.characters[Object.values(chosenUserData.characters).filter(c => c.profiles[message.guild.id] !== undefined).filter(c => {
    ...
    })[0]?._id || chosenUserData?.currentCharacter?.[message.guild.id] || ''] : null, chosenProfileData = chosenCharacterData != null ? chosenCharacterData?.profiles?.[message.guild.id] : null, 
    ...
    async function interactionCollector(message) {
        ...
        if (interaction.isSelectMenu()) {
            if (interaction.values[0] === 'heal_user_page') {
                ...
                if (Object.keys(allHurtCharactersList).length > 0 && userSelectMenu) {
                    componentArray.unshift(new MessageActionRow().addComponents([userSelectMenu]));
                }
                botReply = await interaction.message
            ...
            if (commonPlantsMap.has(interaction.values[0]) || uncommonPlantsMap.has(interaction.values[0]) || rarePlantsMap.has(interaction.values[0]) || interaction.values[0] === 'water') {
                ...
                if (interaction.values[0] === 'water') {
                    ...
                    if (isSuccessful === true) {
                        ...
                        chosenUserData = ( /** @type {import('../../typedef').ProfileSchema} */(await profileModel.findOneAndUpdate({ uuid: chosenUserData.uuid }, (/** @type {import('../../typedef').ProfileSchema} */ cP) => {
                            // @ts-ignore, since chosenUserData and chosenCharacterData can only be null for "heal-user-options"-select menu
                ...
                if (chosenUserData.userId !== userData.userId) {
                    await addFriendshipPoints(message, userData, characterData._id, chosenUserData, chosenCharacterData._id);
                }
                return;
            }
        }
    ...
    async function getUserSelectMenu(message) {
        ...
        const allHurtUsersList = /** @type {Array<import('../../typedef').ProfileSchema>} */ (await profileModel.find((/** @type {import('../../typedef').ProfileSchema} */ u) => {
    ...
    async function getWoundList(healUserData, healCharacterId, message) {
        ...
        const { inventory1Embed, inventory1SelectMenu } = getFirstHealPage(message);
        if (inventory1Embed.fields.length === 0) {
            inventoryPagesButtons[0].disabled = true;
        }
        const embeds = [...embedArray, characterConditionEmbed, ...Object.keys(allHurtCharactersList).length > 0 ? [inventory1Embed] : []], components = [...Object.keys(allHurtCharactersList).length > 0 ?
                [
    ...
    function getFirstHealPage(message) {
        ...
        if (inventory1SelectMenu.options.length === 0) {
            inventory1SelectMenu = null;
        }
