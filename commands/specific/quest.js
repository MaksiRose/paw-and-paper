...
module.exports = {
	...
	async sendMessage(client, message, argumentsArray, profileData, serverData, embedArray) {
		...
		if (checkValidity.hasCooldown(message, profileData)) {
		...
		// I STILL HAVE TO DO A PROMISE TO MAKE SURE YOU CANT DO A NEW COMMAND BEFORE THIS IS DONE
		// PROBLEM: DEALING WITH ASYNC AWAIT WITHIN PROMISE
			...
			*/
			...
			collector.on('end', async collected => {
				...
				if (hitValue >= 10) {
					...
					return botReply = await botReply.edit({
						embeds: embedArray,
						components: [],
					});
				}
