...
export default async function wearDownDen(
	...
	const denStatkind = (['structure', 'bedding', 'thickness', 'evenness'] as const)[generateRandomNumber(4, 0)];
	if (!denStatkind) { throw new TypeError('denStatkind is undefined'); }

	const denWeardownPoints = getSmallerNumber(serverData.dens[denName][denStatkind], generateRandomNumber(5, 1));
