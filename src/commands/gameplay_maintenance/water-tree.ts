...
const userTimeoutMap = new Map<string, NodeJS.Timeout>();
...
export const command: SlashCommand = {
	...
	sendCommand: async (interaction, { user, quid, userToServer, quidToServer, server }) => {
		...
		const queue = getOrCreateQueue(quidToServer);
		await queue.wait();
		try {
...
export async function sendReminder(
	...
	userTimeoutMap.set(quidToServer.quidId + quidToServer.serverId, setTimeout(async () => {
		...
		try {
			...
			// This checks whether the sapling exists, whether the user wants a reminder and whether a reminder has not already been sent. It also checks whether there is a timestmap and whether that timestamp is smaller than or equal to the current time. If it is not, that means that this timeout has been triggered sometime between the user executing the water-tree command, and the water-tree command replacing this timeout. Aka, the reminder and the user executing the command happen simultaneously. To avoid the sentReminder being overwritten here and another reminder not being sent tomorrow, the queue and that part of the if statement are put in place to ensure that this either gets executed before the command and the command waits until this is done, or this is executed after the command is finished (and remember that the queue has to be set inside the timeout so just removing the timeout doesnt stop this from executing) in which case it will be stopped here and not set sentReminder back to true.s
			if (quidToServer.sapling_exists && user.reminders_water && !quidToServer.sapling_sentReminder && quidToServer.sapling_nextWaterTimestamp && quidToServer.sapling_nextWaterTimestamp <= now()) {
...
export function stopReminder(
	...
	if (userTimeoutMap.has(quidId + serverId)) { clearTimeout(userTimeoutMap.get(quidId + serverId)); }
}
...
function getOrCreateQueue(
	...
	return userQueueMap.get(quidToServer.quidId + quidToServer.serverId) ?? userQueueMap.set(quidToServer.quidId + quidToServer.serverId, new AsyncQueue()).get(quidToServer.quidId + quidToServer.serverId)!;
}