...
export const command: SlashCommand = {
	...
	sendCommand: async (interaction, { user, quid, userToServer, quidToServer, server }) => {
		...
		if (quidToServer.sapling.exists === false) {
		...
		if (timeDifference <= thirtyMinutes) {
			...
			quidToServer.sapling.health += saplingHealthPoints;
			quidToServer.sapling.waterCycles += 1;

			healthPoints = pullFromWeightedTable({ 1: 5, 2: 4, 3: 3, 4: 2, 5: 1 }) + getRandomNumber(Math.round(quidToServer.sapling.waterCycles / 4));
		...
		/* This is the second of three `if` statements that check the time difference between the current timestamp and the timestamp of the perfect watering time. If the time difference is less than or equal to 3 hours, the number of watering cycles is increased by 1, the experience points are set to the number of watering cycles, and the embed's description and footer are set. */
			...
			if (timeDifference <= threeHours) { quidToServer.sapling.waterCycles += 1; }
		...
		/* Checking if the sapling is overdue for watering, and if it is, it is calculating how much health it has lost. */
			...
			experiencePoints = 0;
			const weeksAlive = Math.floor(quidToServer.sapling.waterCycles / 7);
			const overdueHours = Math.ceil(timeDifference / oneHour) - 3;
			const percentage = (overdueHours * 3) / 100;
			const lostHealthPoints = Math.round(quidToServer.sapling.health * percentage) + weeksAlive;
			quidToServer.sapling.health -= (quidToServer.sapling.health - lostHealthPoints > 0 ? lostHealthPoints : quidToServer.sapling.health - lostHealthPoints > -weeksAlive ? quidToServer.sapling.health - 1 : quidToServer.sapling.health);
		...
		quidToServer.sapling.nextWaterTimestamp = currentTimestamp + twentyFourHours;
		quidToServer.sapling.lastMessageChannelId = interaction.channelId;
		quidToServer.sapling.sentReminder = false;
		...
		if (quidToServer.sapling.health <= 0) {
...
export async function sendReminder(
	...
	userMap.set(quid.id + quidToServer.serverId, setTimeout(async () => {
		try {
			...
			if (quidToServer.sapling.exists && userData.settings.reminders.water && !quidToServer.sapling.sentReminder) {
				...
				const channel = await client.channels.fetch(quidToServer.sapling.lastMessageChannelId!);
	...
	}, (quidToServer.sapling.nextWaterTimestamp ?? Date.now()) - Date.now()));
}
