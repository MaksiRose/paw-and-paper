...
export async function executeExploring(
	...
	let botReply = await (async function(messageObject) { return buttonInteraction ? await update(buttonInteraction, messageObject) : await respond(interaction, messageObject, true); })(getWaitingMessageObject(messageContent, embedArray, quidData, interaction.guildId, waitingString, waitingGameField, waitingComponent))
		.catch((error) => { throw new Error(error); });
	...
	collector.on('collect', async int => {
		...
		botReply = await update(int, getWaitingMessageObject(messageContent, embedArray, quidData, interaction.guildId, waitingString, waitingGameField, waitingComponent))
			.catch((error) => {
				if (error.httpStatus !== 404) { throw new Error(error); }
				return botReply;
			});
