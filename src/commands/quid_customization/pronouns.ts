...
export async function pronounsInteractionCollector(
	...
	if (interaction.isSelectMenu() && interaction.customId.includes('selectmodal')) {
		...
		const _userData = await userModel.findOne(u => Object.keys(u.quids).includes(_id));
		const userData = getUserData(_userData, interaction.guildId || 'DM', getMapData(_userData.quids, _id));
